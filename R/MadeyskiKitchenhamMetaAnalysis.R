...
reproduceTableWithSourceDataByCiolkowski <- function(){
  ...
  print(xtable::xtable(reproducer::Ciolkowski09ESEM.MetaAnalysis.PBRvsCBRorAR), type="latex")
}
...
reproduceTableWithEffectSizesBasedOnMeanDifferences <- function() {
  ...
  print(xtable::xtable(dataMK), type="latex")
}
...
reproduceMixedEffectsAnalysisWithEstimatedVarianceAndExperimentalDesignModerator <- function() {
  dataMK <- reproducer::MadeyskiKitchenham.MetaAnalysis.PBRvsCBRorAR
  resWithExpDesignUsingSubjectSpecificVarianceMod <- metafor::rma(yi = dataMK$Diff, vi = dataMK$V_Alt, data = dataMK, method = "REML", mods = ~ factor(ExpDesign) -
    1)
...
getSimulationData <- function(var, covar, meanA1, treatmentDiff, periodEffect, numOfSamples) {
  # Covariance matrix r=.75 var=25 covar=18.75
  sigmaA <- matrix(c(var, covar, covar, var), nrow = 2, ncol = 2)

  # Mean for treatment sequence B common mean mu=50, treatment difference = 10, period effect=5 meanB=c(50,65) SG2
  meanB <- c(meanA1, meanA1 + treatmentDiff + periodEffect)
  # Mean for sequence A meanA=c(60,55) SG1
...
plotOutcomesForIndividualsInEachSequenceGroup <- function(var, covar, meanA1, treatmentDiff, periodEffect, numOfSamples) {
  ...
  graphics::par(mfrow = c(1, 2), cex = 0.75)
  graphics::boxplot(Difference ~ Sequence,
    data = newAll, ylab = "Subject Difference for each Sequence", xlab = "Sequence ID", main = "(b) Crossover Differences for each Sequence",
    cex = 0.8
  )

  graphics::plot(newdataT1$pid, newdataT1$result,
    ylim = c(30, 80), xlim = c(1, 30), ylab = "Outcome", xlab = "Subject ID", pch = 0, main = "(a) Raw Data for Each Individual",
    typ = "b"
  )
...
getEffectSizesABBA <- function(simulationData) {
  # This function calculates the correct variances for all AB/BA standardized effect sizes for small and medium size samples.
  ...
  # effectSizes<-dgta.frame(unstandardizedES, r, t,t.unbiased,var.t, var.t.unbiased,dRM, dRM.Fromt, dRM.Fromt.unbiased,
  # gRM,var.dRM.Fromt,var.dRM.Fromt.unbiased,var.dRM.biased, var.dRM.unbiased, var.gRM, var.gRM.Fromt,
  # var.gRM.Fromt.unbiased,var.gRM.From.vardRM.unbiased, dIG, dIG.Fromt, dIG.Fromt.unbiased, var.dIG.Fromt,
  # var.dIG.Fromt.unbiased,var.dIG.biased,var.dIG.unbiased, gIG, var.gIG.Fromt, var.gIG.Fromt.unbiased, var.gIG.From.vardIG.unbiased, var.gIG,
  # var.t.Approx, var.t.Approx.unbiased, var.gRM.Approx,var.gIG.Approx, var.dRM.Approx.alternative, var.dIG.Approx.alternative,PRE.vardrm,
  # PRE.vardig, PRE.vargrm,PRE.vargig,var.sig, var.within, var.between)

  effectSizes <- data.frame(
    unstandardizedES, r, t, t.unbiased, var.t.unbiased, dRM, var.dRM.unbiased, gRM, var.gRM, dIG, var.dIG.unbiased, gIG, var.gIG,
    var.t.Approx, var.t.Approx.unbiased, var.gRM.Approx, var.gIG.Approx, var.dRM.Approx.alternative, var.dIG.Approx.alternative, PRE.vardrm, PRE.vardig,
    PRE.vargrm, PRE.vargig, var.sig, var.within, var.between
  )
  return(effectSizes)
}
...
getEffectSizesABBAIgnoringPeriodEffect <- function(simulationData) {
  ...
  # d<-cbind(dIG,dRM) return(d)
  effectSizes.ipe <- data.frame(
    dIG.ipe, var.dIG.ipe, dRM.ipe, var.dRM.ipe, dIG.Fromt.ipe, var.dIG.Fromt.ipe, dRM.Fromt.ipe, var.dRM.Fromt.ipe, var.dRM.Fromt2.ipe,
    unstandardizedES.ipe, var.sig.ipe, var.within.ipe, var.between.ipe, t.ipe, var.t.ipe
  )
...
reproduceSimulationResultsBasedOn500Reps1000Obs <- function() {
  ...
  # Covariance matrix r=.75
  var <- 25
  covar <- 18.75

  sigmaA <- matrix(c(var, covar, covar, var), nrow = 2, ncol = 2)

  # Mean for treatment sequence A common mean mu=50, treatment difference = 10, period effect=5
  meanA <- c(50, 65)
  # Mean for sequence B
  meanB <- c(60, 55)
  ...
  simultationResults <- data.frame(
    treatmentEffect.Ave, dRM.Ave, dRM.Var, dRM.Var.Ave, dRM.Var.ModerateSampleSizeApprox, dIG.Ave, dIG.Var, dIG.Var.Ave,
    dIG.Var.ModerateSampleSizeApprox
  ) # removed ', dIG.Var.CurtinAve)' since 0.1.7
...
percentageInaccuracyOfLargeSampleVarianceApproximation <- function(data) {
  ...
  graphics::par(mfrow = c(2, 2), mar = c(5, 6, 4, 2) + 0.1)

  d <- data[c(1:7), ]
  graphics::plot(d$actualSampleSize, d$Accuracy,
    ylab = "Approx Variance\n Inaccuracy with r=0", xlab = "Sample Size", type = "b", pch = 0, lty = 1,
    ylim = c(0, 30), cex.sub = 0.8, main = "(a)"
  )
  ...
  graphics::legend("topright", inset = 0.01, title = "Effect Size", c("0", "2.5", "5"), lty = c(1, 2, 3), pch = c(0, 1, 2), cex = 0.7)

  d <- data[c(22:28), ]
  graphics::plot(d$actualSampleSize, d$Accuracy,
    ylab = "Approx Variance Inaccuracy with r=0.25", xlab = "Sample Size", type = "b", pch = 0, lty = 1,
    ylim = c(0, 40), cex.sub = 0.8, main = "(b)"
  )
  ...
  d <- data[c(43:49), ]
  graphics::plot(d$actualSampleSize, d$Accuracy,
    ylab = "Approx Variance Inaccuracy with r=0.5", xlab = "Sample Size", type = "b", pch = 0, lty = 1,
    ylim = c(0, 40), cex.sub = 0.8, main = "(c)"
  )
  ...
  d <- data[c(64:70), ]
  graphics::plot(d$actualSampleSize, d$Accuracy,
    ylab = "Approx Variance Inaccuracy with r=0.75", xlab = "Sample Size", type = "b", pch = 0, lty = 1,
    ylim = c(0, 40), cex.sub = 0.8, main = "(d)"
  )
...
proportionOfSignificantTValuesUsingCorrectAnalysis <- function(data) {
  ...
  d <- data[c(1:7), ]
  graphics::plot(d$actualSampleSize, d$PropSig, ylab = "Prop Significant t-Values with r=0", xlab = "Sample Size", type = "b", pch = 0, lty = 1, ylim = c(
    0,
    100
  ), xlim = c(0, 100), cex.sub = 0.8, main = "(a)")
  ...
  graphics::legend("right", inset = 0.01, title = "Effect Size", c("0", "2.5", "5"), lty = c(1, 2, 3), pch = c(0, 1, 2), cex = 0.7)

  d <- data[c(22:28), ]
  graphics::plot(d$actualSampleSize, d$PropSig,
    ylab = "Prop Significant t-Values with r=0.25", xlab = "Sample Size", type = "b", pch = 0, lty = 1,
    ylim = c(0, 100), xlim = c(0, 100), cex.sub = 0.8, main = "(b)"
  )
  ...
  d <- data[c(43:49), ]
  graphics::plot(d$actualSampleSize, d$PropSig,
    ylab = "Prop Significant t-Values with r=0.5", xlab = "Sample Size", type = "b", pch = 0, lty = 1,
    ylim = c(0, 100), xlim = c(0, 100), cex.sub = 0.8, main = "(c)"
  )
  ...
  d <- data[c(64:70), ]
  graphics::plot(d$actualSampleSize, d$PropSig,
    ylab = "Prop Significant t-Values with r=0.75", xlab = "Sample Size", type = "b", pch = 0, lty = 1,
    ylim = c(0, 100), xlim = c(0, 100), cex.sub = 0.8, main = "(d)"
  )
...
proportionOfSignificantTValuesUsingIncorrectAnalysis <- function(data) {
  ...
  d <- data[c(1:7), ]
  graphics::plot(d$actualSampleSize, d$WrongTSig,
    ylab = "Prop Significant Incorrect t-Values with r=0", xlab = "Sample Size", type = "b", pch = 0,
    lty = 1, ylim = c(0, 100), xlim = c(0, 100), cex.sub = 0.8, main = "(a)"
  )
  ...
  d <- data[c(22:28), ]
  graphics::plot(d$actualSampleSize, d$WrongTSig,
    ylab = "Prop Significant Incorrect t-Values with r=0.25", xlab = "Sample Size", type = "b", pch = 0,
    lty = 1, ylim = c(0, 100), xlim = c(0, 100), cex.sub = 0.8, main = "(b)"
  )
  ...
  d <- data[c(43:49), ]
  graphics::plot(d$actualSampleSize, d$WrongTSig,
    ylab = "Prop Significant Incorrect t-Values with r=0.5", xlab = "Sample Size", type = "b", pch = 0,
    lty = 1, ylim = c(0, 100), xlim = c(0, 100), cex.sub = 0.8, main = "(c)"
  )
  ...
  d <- data[c(64:70), ]
  graphics::plot(d$actualSampleSize, d$WrongTSig,
    ylab = "Prop Significant Incorrect t-Values with r=0.75", xlab = "Sample Size", type = "b", pch = 0,
    lty = 1, ylim = c(0, 100), xlim = c(0, 100), cex.sub = 0.8, main = "(d)"
  )
...
effectSizeCI <- function(expDesign, t, n1, n2, r = 0, epsilon = 1e-10, maxsteps = 1000, stepsize = 3) {
  # stepsize=3 #influences the number of steps required to obtain the final result (i.e., the result of precison defined by the epsilon), e.g.,
  # consider stepsize=2
  if (t > 37.62) {
    t <- 37.62
  }
  ...
  while (i <= maxsteps & (abs(pt_LB) >= abs(0.025 + epsilon) | abs(pt_LB) <= abs(0.025 - epsilon) | abs(pt_UB) >= abs(0.975 + epsilon) | abs(pt_UB) <=
    abs(0.975 - epsilon))) {
    if (i == 1) {
      ...
      if (abs(pt_LB) <= abs(0.025 - epsilon)) {
        if (t_LB > 0) {
          # we need larger value of t_LB
          t_LB <- (1 + (stepsize * abs(pt_LB - 0.025))) * t_LB
        } else {
          # we need smaller value of t_LB
          t_LB <- (1 - (stepsize * abs(pt_LB - 0.025))) * t_LB
        }
      }
      if (abs(pt_LB) >= abs(0.025 + epsilon)) {
        if (t_LB > 0) {
          ...
          # we need a larger value of T_LB
          t_LB <- (1 + (stepsize * abs(pt_LB - 0.025))) * t_LB
        }
      }
      ...
      if (abs(pt_UB) <= abs(0.975 - epsilon)) {
        if (t_UB > 0) {
          # we need larger value of t_UB
          t_UB <- (1 + (stepsize * abs(pt_UB - 0.975))) * t_UB
        } else {
          # we need a smaller value of t_UB
          t_UB <- (1 - (stepsize * abs(pt_UB - 0.975))) * t_UB
        }
      }
      if (abs(pt_UB) >= abs(0.975 + epsilon))
      # we need smaler value of t_LB
        {
          if (t_UB > 0) {
            # we need smaller value of t_UB
            t_UB <- (1 - (stepsize * abs(pt_UB - 0.975))) * t_UB
          } else {
            # we need a larger value of T_UB
            t_UB <- (1 + (stepsize * abs(pt_UB - 0.975))) * t_UB
          }
        }
    }
  ...
  result <- list(
    t_LB = t_LB, t_UB = t_UB, d_RM_LB = d_RM_LB, d_RM_UB = d_RM_UB, d_IG_LB = d_IG_LB, d_IG_UB = d_IG_UB, g_RM_LB = g_RM_LB, g_RM_UB = g_RM_UB,
    g_IG_LB = g_IG_LB, g_IG_UB = g_IG_UB, i = i
  )
