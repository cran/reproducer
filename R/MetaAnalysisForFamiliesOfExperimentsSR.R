## reproducer package for R Copyright (C) 2018 Lech Madeyski and Barbara Kitchenham R functions to complement paper by Barbara Kitchenham, Lech
## Madeyski, Pearl Brereton, 'Meta-analysis for Families of Experiments: A Systematic Review and Reproducibility Assessment'.  This file includes
## functions that, e.g., allow to construct and format five of the output tables used in the systematic review paper. It extracts the reported
## values for effect sizes, meta-analysis and descriptive statistics in the primary studies from txt files. It uses the descriptive statistics to
## re-calculate effect sizes and then performs a meta-analyses using the constructed effect sizes and compares the calculated values with the
## reported values.
...
PrepareForMetaAnalysisGtoR <- function(Mc, Mt, SDc, SDt, Nc, Nt) {
  ...
  results <- data.frame(HGvalues$Hg, HGvalues$HgAdjusted, Hgupper, Hglower, zr, vi, r, pvalue)
  return(results)
}
...
ExtractMAStatistics <- function(maresults, Nc, Nt, Transform = TRUE, type = "d", sig = 4) {
...
reproduceTablesOfPaperMetaAnalysisForFamiliesOfExperiments <- function() {
  # ExperimentEffectSizes holds the data for all studies that reported expeirment level effect sizes. Note S3 only reported experiment effect
  # sizes for one of its experiments. ABBAEffectSizes holds the effect sizes for the three papers that reported results on a time
  # period/document bais.  ExperimentEffectSizes=read.table(paste0(dataSetPath, 'ReportedEffectSizes.txt'),header=TRUE)
  ...
  # Read files that contain the basic statistics. ExpData holds the data for all the experiments. Notes values in ExpData for studies 3, 7, 11
  # were gernerated from the data in DocData. DocData holds the basic statistics for the three papers that reported effect sizes for time
  # periods not for experiments.
  ...
  # *********************************************************** This code calculates effect sizes and meta-analysis from the baic statistics and
  # builds up tables to compare the calculated results with the reported results.

  # *********************************************************** Study1:S1 Abrahao 2013 Study reported 5 experiments all using a 4-group
  # crossover design
  ...
  S1EffectSizes <- PrepareForMetaAnalysisGtoR(S1data$Mc, S1data$Mt, S1data$SDc, S1data$SDt, S1data$Nc, S1data$Nt)
  # Do a random effect meta-analysis of the transformed r_pbs effect size
  S1MA <- metafor::rma(S1EffectSizes$zr, S1EffectSizes$vi)
  ...
  # Append the calculated meta-analysis statistics and the reported meta-analysis statistics to the file MAStats that will comprise the basic
  # meta-analysis table in the SR paper.
  ...
  # Append the entries for Study 1, to the data.frame used to hold the comparison of the calculated and reported effect sizes.  Fist construct a
  # single row data.frame of the calculated effect sizes.  BAK Changed Type to 'gIG'
  ESdata1 <- data.frame(
    Study = Study, Type = "gIG", Source = "Calc", Design = "4GroupCO", Exp1 = S1.ES[1], Exp2 = S1.ES[2], Exp3 = S1.ES[3], Exp4 = S1.ES[4],
    Exp5 = S1.ES[5], RR = NA
  )

  # Next obtain the reported effect sizes from the data.frame
  S1.ESrep <- subset(ExperimentEffectSizes, ExperimentEffectSizes$Study == "S1")
  ...
  # Next check whether the effect sizes have been reproduced. Look at the difference between the rported and calculated effect sizes. An
  # absolute difference less than or equal to 0.05 is our definition of reproduced.
  ...
  # *********************************************************** Study: S2 Scanniello 2014. Study reported 4 experiments all using a 4-group
  # crossover design
  ...
  S2EffectSizes <- PrepareForMetaAnalysisGtoR(S2data$Mc, S2data$Mt, S2data$SDc, S2data$SDt, S2data$Nc, S2data$Nt)
  ...
  ESdata1 <- data.frame(
    Study = Study, Type = "gIG", Source = "Calc", Design = "4GroupCO", Exp1 = S2.ES[1], Exp2 = S2.ES[2], Exp3 = S2.ES[3], Exp4 = S2.ES[4],
    Exp5 = S2.ES[5], RR = NA
  )
  ...
  # Study 3: S3 Cruz-Lemus 2009. The authors reported the results of 5 experiments. The first four experiments used an ABBA crossover design.
  # The final experiment used an independent groups design. For the first four experiments, the authors reported the basic statistics for each
  # document, so the descriptive data needed to be re-calculated for each technique group. They undertook their meta-analysis on the basis of
  # the results for each domain (i.e. document). Comoarison of effect sizes will be done on the for the first document data only.

  # Extract the experiment basic statics, for the overall experiment and the first document

  S3data <- subset(ExpData, ExpData == "S3")

  Doc1Data <- subset(DocData, DocData$Study == "S3" & DocData$Doc == "Doc1")


  if (length(S3data$Study) < 5) {
    # The data for S3 has been changed and the values for experiments 1 to 4 need to be recalculated from the data for each document. This
    # requires the integrating the document 1 and document 2 data into an effect size for each experiment.
    Doc2Data <- subset(DocData, DocData$Study == "S3" & DocData$Doc == "Doc2")

    c.Data <- aggregateIndividualDocumentStatistics(Doc1Data$Mc, Doc1Data$SDc, Doc1Data$Nc, Doc2Data$Mc, Doc2Data$SDc, Doc2Data$Nc)
    t.Data <- aggregateIndividualDocumentStatistics(Doc1Data$Mt, Doc1Data$SDt, Doc1Data$Nt, Doc2Data$Mt, Doc2Data$SDt, Doc2Data$Nt)
    r <- rep("NA", 4)
    Study <- rep("S3", 4)
    Exp4 <- c("Exp1", "Exp2", "Exp3", "Exp4")

    S3extradata <- data.frame(Study = Study, Exp = Exp4, Mc = signif(c.Data$M, 4), SDc = signif(c.Data$S, 6), Nc = Doc1Data$Nc, Mt = signif(
      t.Data$M,
      4
    ), SDt = signif(t.Data$S, 6), Nt = Doc1Data$Nt, r = r)
    # Store the recalculated data values back into the ExpData file, so they dont need to be recalculated again unless the document values
    # chnage again.
    ExpData <- rbind(ExpData, S3extradata)
    S3data <- rbind(S3extradata, S3data)
  }
  # Calculate the Hg effect sizes that will be used for comparison with the reported effect sizes and the transformed r statistics that will be
  # used for the meta-analysis
  S3EffectSizes <- PrepareForMetaAnalysisGtoR(S3data$Mc, S3data$Mt, S3data$SDc, S3data$SDt, S3data$Nc, S3data$Nt)
  ...
  # Construct the entry for the calculated meta-analysis results that will be stored in the data.frame holding the meta-analysis results for all
  ...
  ESdata1 <- data.frame(
    Study = Study, Type = "gIG", Source = "Calc", Design = "Mixed", Exp1 = S3.ES[1], Exp2 = S3.ES[2], Exp3 = S3.ES[3], Exp4 = S3.ES[4],
    Exp5 = S3.ES[5], RR = NA
  )

  # Extract the reported experiment effect sizes. Only experiment 5 data exists for S3
  S3ESRep <- subset(ExperimentEffectSizes, ExperimentEffectSizes$Study == "S3")
  ...
  # Doc1.Data holds the basic data for the 4 experiments that used an ABBA crossover. Add the entry for the 5th experiment to the document 1
  # data
  ...
  S3Doc1EffectSizes <- PrepareForMetaAnalysisGtoR(Doc1.Data$Mc, Doc1.Data$Mt, Doc1.Data$SDc, Doc1.Data$SDt, Doc1.Data$Nc, Doc1.Data$Nt)
  ...
  # Create the first entry in the MAStatsTP1 that will hold the meta-analysis results for the studies that reported document/time period based
  # effect sizes.
  Study <- "S3"
  ...
  ESTP1res <- data.frame(
    Study = Study, Type = "gIG", Design = "Mixed", Source = "Calc", Exp1 = S3Doc1.ES[1], Exp2 = S3Doc1.ES[2], Exp3 = S3Doc1.ES[3],
    Exp4 = S3Doc1.ES[4], Exp5 = S3Doc1.ES[5]
  )
  ...
  # ************************************************************************* Study 4: S4. Fernandez 2013 reported resuls for 3 experiments al.l
  # of which used a 4-group ABBA crossover design

  # Obtain basic statistics for all experiments.
  S4data <- subset(ExpData, ExpData == "S4")

  # Calculate effect sizes for meta-analysis
  S4EffectSizes <- PrepareForMetaAnalysisGtoR(S4data$Mc, S4data$Mt, S4data$SDc, S4data$SDt, S4data$Nc, S4data$Nt)
  ...
  ESdata1 <- data.frame(
    Study = Study, Type = "gIG", Source = "Calc", Design = "4GroupCO", Exp1 = S4.ES[1], Exp2 = S4.ES[2], Exp3 = S4.ES[3], Exp4 = NA,
    Exp5 = NA, RR = NA
  )
  ...
  # *********************************************************** Fernandez-Saez 2016 (pre-publication date 2014) Reported 5 experiments all using
  # the 4-group crossover design.

  # Get the reported basic statsitics for Study 5 experiments
  S5data <- subset(ExpData, ExpData == "S5")

  # Calculate the effect sizes
  S5EffectSizes <- PrepareForMetaAnalysisGtoR(S5data$Mc, S5data$Mt, S5data$SDc, S5data$SDt, S5data$Nc, S5data$Nt)
  ...
  ESdata1 <- data.frame(
    Study = Study, Type = "gIG", Source = "Calc", Design = "4GroupCO", Exp1 = S5.ES[1], Exp2 = S5.ES[2], Exp3 = S5.ES[3], Exp4 = S5.ES[4],
    Exp5 = S5.ES[5], RR = NA
  )

  # Index correct table entry
  S5EffectSizes <- subset(ExperimentEffectSizes, ExperimentEffectSizes$Study == "S5")
  ...
  # Obtain effect sizes
  S6EffectSizes <- PrepareForMetaAnalysisGtoR(S6data$Mc, S6data$Mt, S6data$SDc, S6data$SDt, S6data$Nc, S6data$Nt)

  # Effect sizes confirm that Hadar used the mean difference divided by the pooled within groups SD. However, we still use the small sample
  # correction.
  ...
  ESdata1 <- data.frame(
    Study = Study, Type = "gIG", Source = "Calc", Design = "4GroupCO", Exp1 = S6.ES[1], Exp2 = S6.ES[2], Exp3 = S6.ES[3], Exp4 = NA,
    Exp5 = NA, RR = NA
  )

  S6Rep.ES <- subset(ExperimentEffectSizes, ExperimentEffectSizes$Study == "S6")
  S6Rep.ES$Type <- "dIG"
  ...
  # ************************************************************* Study 7: S7. Teruel 2012. The authors reported results of 3 experiments all
  # using an ABBA crossover design. The authors obtained effect sizes for each document believeing they wanted to aggregate effect sizes for
  # each application.  Since they used an AB/BA crossover, the results for the first document (Jigsaw) could be used as a between groups study.
  ...
  Doc1data <- subset(DocData, DocData$Study == "S7" & DocData$Doc == "Doc1")

  if (length(S7data$Study) < 3) {
    # The Doc data has been revised and the calculated basic statistics need to be recalculated
    Doc2data <- subset(DocData, DocData$Study == "S7" & DocData$Doc == "Doc2")
    ...
    S7data <- data.frame(Study = Study, Exp = Exp3, Mc = signif(c.Data$M, 4), SDc = signif(c.Data$SD, 6), Nc = Doc1data$Nc, Mt = signif(
      t.Data$M,
      4
    ), SDt = signif(t.Data$SD, 6), Nt = Doc1data$Nt, r = r3)


    ExpData <- rbind(ExpData, S7data)
  }

  S7EffectSizes <- PrepareForMetaAnalysisGtoR(S7data$Mc, S7data$Mt, S7data$SDc, S7data$SDt, S7data$Nc, S7data$Nt)
  ...
  ESdata1 <- data.frame(
    Study = Study, Type = "gIG", Source = "Calc", Design = "ABBACO", Exp1 = S7.ES[1], Exp2 = S7.ES[2], Exp3 = S7.ES[3], Exp4 = NA,
    Exp5 = NA, RR = NA
  )
  ...
  # S7ES=subset(ExperimentEffectSizes,ExperimentEffectSizes$Study=='S7') ESdata1=rbind(ESdata1,cbind(S7ES,RR=NA))

  ESdata <- rbind(ESdata, ESdata1)

  # Analyse the first time period/doc1 results for reproducibility. The document 1 only results are analysed as an independent groups study. For
  # this analysis we use the Hg values directly and the appropriate Hg variance


  S7TP1 <- PrepareForMetaAnalysisGtoR(Doc1data$Mc, Doc1data$Mt, Doc1data$SDc, Doc1data$SDt, Doc1data$Nc, Doc1data$Nt)

  # Construct normal approximation variance of the effect size
  c.Doc1 <- calculateSmallSampleSizeAdjustment(Doc1data$Nc + Doc1data$Nt - 2)

  # Use the adjusted Hg but the standard normal approximation equation, which is (n1+n2)/(n1*n2)+ g^2/(2c^2(n1+n2))
  Doc1.VdUnadjusted <- ((Doc1data$Nc + Doc1data$Nt) / (Doc1data$Nc * Doc1data$Nt) + S7TP1$HGvalues.HgAdjusted^2 / (2 * c.Doc1^2 * (Doc1data$Nc + Doc1data$Nt)))

  # Do a meta-analysis of the adjusted Hg values using the ML method
  S7TP1res <- metafor::rma(S7TP1$HGvalues.Hg, Doc1.VdUnadjusted, method = "ML")

  # Copy the meta-analysis resutls & effect sizes into the data.frames MAStatsTP1 and ESTP1res which hold the data for studies that aggregated
  # results on document basis

  S7TP1MAStats <- ExtractMAStatistics(S7TP1res, sum(Doc1data$Nc), sum(Doc1data$Nt), Transform = F, sig = 5)

  MAStatsTP1 <- rbind(MAStatsTP1, cbind(Study, Type = "d", Source = "Calc", S7TP1MAStats))
  ...
  ESTP1res <- rbind(ESTP1res, data.frame(Study,
    Type = "d", Source = "Calc", Design = "ABBACO", Exp1 = S7TP1Hg[1], Exp2 = S7TP1Hg[2], Exp3 = S7TP1Hg[3],
    Exp4 = NA, Exp5 = NA
  ...
  # *********************************************************** Study 8: S8.Gonzalez-Huerta, 2015. This paper used the point bi-serial
  # correlation effect size. The authors reported the results of 4 experiments all using a 4-group crossover design. They used p-values to
  # calculate r_pb.


  S8data <- subset(ExpData, ExpData$Study == "S8")

  S8EffectSizes <- PrepareForMetaAnalysisGtoR(S8data$Mc, S8data$Mt, S8data$SDc, S8data$SDt, S8data$Nt, S8data$Nc)

  S8MAres <- metafor::rma(S8EffectSizes$zr, S8EffectSizes$vi)

  # Transform the summary statistics back to r values

  S8MAstatistics <- ExtractMAStatistics(S8MAres, sum(S8data$Nt), sum(S8data$Nt), type = "r")
  ...
  Type <- "r"
  Source <- "Calc"
  MAStats <- rbind(MAStats, cbind(Study, Type = "rpb", Source, S8MAstatistics, RR = NA))
  ...
  ESdata1 <- data.frame(
    Study = Study, Type = "rpb", Source = "Calc", Design = "4GroupCO", Exp1 = S8.ES[1], Exp2 = S8.ES[2], Exp3 = S8.ES[3], Exp4 = S8.ES[4],
    Exp5 = NA, RR = NA
  )

  # Index correct table entry
  S8ESrep <- subset(ExperimentEffectSizes, ExperimentEffectSizes$Study == "S8" & Type == "r")
  S8ESrep$Type <- "rpb"
  ...
  # Reported p-values.Checking the r-values calculated by GH using the method of converting p-values to Z values and then coverting to r using
  # r=Z/sqrt(N)

  S8ESprep <- subset(ExperimentEffectSizes, ExperimentEffectSizes$Study == "S8" & Type == "p")
  ...
  GH2015extra <- data.frame(rbind(p = S8ESp, Z = signif(S8p.Z, 4), r.NP = signif(S8r.NP, 4), r.NO = signif(S8r.NO, 4)))
  ...
  GH2015extra <- reshape::rename(GH2015extra, c(newColumnStatistic = "Statistic", X1 = "Exp1", X2 = "Exp2", X3 = "Exp3", X4 = "Exp4"))


  # ************************************************************* Study 9: S9. Fernandez-Saez 2015. The experiments in this family are all
  # between groups. We could use the correct Hedge's g variance from the reported data. However, for consistency we use the transformation
  # approach. We assume FD is the control and RE is the treatment.

  S9data <- subset(ExpData, ExpData$Study == "S9")

  S9EffectSizes <- PrepareForMetaAnalysisGtoR(S9data$Mc, S9data$Mt, S9data$SDc, S9data$SDt, S9data$Nc, S9data$Nt)
  ...
  ESdata1 <- data.frame(
    Study = Study, Type = Type, Source = "Calc", Design = "IndGroups", Exp1 = S9.ES[1], Exp2 = S9.ES[2], Exp3 = S9.ES[3], Exp4 = S9.ES[4],
    Exp5 = S9.ES[5], RR = NA
  )

  # Extract reported effect sizes

  S9ESRep <- subset(ExperimentEffectSizes, ExperimentEffectSizes$Study == "S9")
  S9ESRep$Type <- "dIG"
  ...
  # ************************************************************* Study S10: S10. Cruz-Lemus 2011. Reported resutls from 3 experiments. All
  # experiments used 4 group crossover design. They did not report effect size values for individual experiments.


  S10data <- subset(ExpData, ExpData$Study == "S10")

  S10.EffectSizes <- PrepareForMetaAnalysisGtoR(S10data$Mc, S10data$Mt, S10data$SDc, S10data$SDt, S10data$Nc, S10data$Nt)
  ...
  ESdata1 <- data.frame(
    Study = Study, Type = "gIG", Source = "Calc", Design = "4GroupCO", Exp1 = S10.ES[1], Exp2 = S10.ES[2], Exp3 = S10.ES[3], Exp4 = NA,
    Exp5 = NA, RR = NA
  )
  ...
  # ************************************************************* Morales-2016 Morales et al. calculated effect sizes for each application. The
  # data averaged over the difference applications are to be found in DescriptiveStatistics.xlsx sheet Morales 2016. The values below were
  # extracted from DescriptiveStatistics.xlsx sheet BasicStatistics, row Morales-2016. The analysis is the same as it was for Tuerel.

  S11data <- subset(ExpData, ExpData$Study == "S11")

  Doc1data <- subset(DocData, DocData$Study == "S11" & DocData$Doc == "Doc1")

  if (length(S11data$data) < 3) {
    Doc2data <- subset(DocData, DocData$Study == "S11" & DocData$Doc == "Doc2")
    ...
    Exp3 <- c("Exp1", "Exp2", "Exp3")
    r3 <- rep(NA, 3)
    S11data <- data.frame(Study = Study, Exp = Exp3, Mc = signif(c.Data$M, 4), SDc = signif(c.Data$SD, 6), Nc = Doc1data$Nc, Mt = signif(
      t.Data$M,
      4
    ), SDt = signif(t.Data$SD, 6), Nt = Doc1data$Nt, r = r3)
  ...
  S11.EffectSizes <- PrepareForMetaAnalysisGtoR(S11data$Mc, S11data$Mt, S11data$SDc, S11data$SDt, S11data$Nc, S11data$Nt)
  ...
  ESdata1 <- data.frame(
    Study = Study, Type = Type, Source = "Calc", Design = "ABBACO", Exp1 = S11.ES[1], Exp2 = S11.ES[2], Exp3 = S11.ES[3], Exp4 = S11.ES[4],
    Exp5 = S11.ES[5], RR = NA
  )
  ...
  S11Doc1.EffectSizes <- PrepareForMetaAnalysisGtoR(Doc1data$Mc, Doc1data$Mt, Doc1data$SDc, Doc1data$SDt, Doc1data$Nc, Doc1data$Nt)

  S11Doc1.c <- calculateSmallSampleSizeAdjustment(Doc1data$Nc + Doc1data$Nt - 2)
  S11Doc1.Vd <- S11Doc1.c^2 * ((Doc1data$Nc + Doc1data$Nt) / (Doc1data$Nc * Doc1data$Nt) + S11Doc1.EffectSizes$HGvalues.HgAdjusted^2 / (2 * (Doc1data$Nc +
    Doc1data$Nt)))
  # sqrt(Drone.Vd) Morales et al.report the standardized mean difference adjusted for small sample sizes in Table 19. They report an analysis
  # based on the unadjusted standardized effect size d_{IG} and the medium sample size variance of d_{IG}

  S11Doc1.VdUnadjusted <- ((Doc1data$Nc + Doc1data$Nt) / (Doc1data$Nc * Doc1data$Nt) + S11Doc1.EffectSizes$HGvalues.HgAdjusted^2 / (2 * S11Doc1.c^2 *
    (Doc1data$Nc + Doc1data$Nt)))

  S11Doc1MAres <- metafor::rma(S11Doc1.EffectSizes$HGvalues.Hg, S11Doc1.VdUnadjusted, method = "ML")
  S11MATP1Stats <- ExtractMAStatistics(S11Doc1MAres, sum(Doc1data$Nc), sum(Doc1data$Nt), Transform = F, sig = 5)

  MAStatsTP1 <- rbind(MAStatsTP1, cbind(Study, Type = "d", Source = "Calc", S11MATP1Stats))
  ...
  ESTP1res <- rbind(ESTP1res, data.frame(Study,
    Type = "d", Source = "Calc", Design = "ABBACO", Exp1 = S11TP1.Hg[1], Exp2 = S11TP1.Hg[2], Exp3 = S11TP1.Hg[3],
    Exp4 = NA, Exp5 = NA
  ...
  # ************************************************************* Study 13: S13. Laitenberger 2001 Reported the correct formula for the
  # individual improvement effect size. They reported the correlation between repeated. Their meta-analysis aggregated p-values using the
  # correct formula. I suggest we do a meta-analysis based the repeated measures effect size and the correct variance. Also since they reported
  # r, we calculate the standardized mean difference effect size equivalent to a between groups study and do a meta-analysis of that effect size
  # to demonstrate the difference.  This analysis is based on the values estimated from the boxplots

  S13data <- subset(ExpData, ExpData$Study == "S13")

  # The study aggregates the p-values of the gRM estimates. We need to compare an aggregation of the gRM estimates to compare the overall
  # p-values

  S13.EffectSizes <- PrepareForMetaAnalysisGtoR(S13data$Mc, S13data$Mt, S13data$SDc, S13data$SDt, S13data$Nc, S13data$Nt)
  ...
  Type <- "gRM"
  Source <- "Calc"
  MAStats <- rbind(MAStats, cbind(Study, Type, Source, S13RMMAStats, RR = NA))
  # Index correct table entry Note there is no study 12, entry 12 applies to study12
  S13RMMArep <- subset(MARepResShort, MARepResShort$Study == "S13" & MARepResShort$Type == "gRM")
  ...
  ESdata2 <- data.frame(
    Study = Study, Type = "gRM", Source = "Calc", Design = "ABBACO", Exp1 = S13RM.ES[1], Exp2 = S13RM.ES[2], Exp3 = S13RM.ES[3],
    Exp4 = NA, Exp5 = NA, RR = NA
  )

  S13RMEffectSizes <- subset(ExperimentEffectSizes, ExperimentEffectSizes$Study == "S13" & Type == "gRM")
  # Index correct table entry
  ...
  ESdata1 <- data.frame(
    Study = Study, Type = "gIG", Source = "Calc", Design = "ABBACO", Exp1 = S13.gIG[1], Exp2 = S13.gIG[2], Exp3 = S13.gIG[3],
    Exp4 = NA, Exp5 = NA, RR = NA
  )

  S13EffectSizes <- subset(ExperimentEffectSizes, ExperimentEffectSizes$Study == "S13" & Type == "gIG")
  ...
  MAStats <- rbind(MAStats, data.frame(Study,
    Type = "P", Source = "Calc", mean = signif(S13.P, 4), pvalue = signif(S13.P.pvalue, 4), UB = NA, LB = NA,
    QE = NA, QEp = NA, RR = NA
  ...
  MAReproduced <- NA
  S13PRep <- subset(MARepResShort, MARepResShort$Study == "S13" & MARepResShort$Type == "P")
  MAStats <- rbind(MAStats, cbind(S13PRep, RR = MAReproduced))

  S13.pt <- signif(S13.pt, 4)

  ESdata3 <- data.frame(
    Study = Study, Type = "p", Source = "Calc", Design = "ABBACO", Exp1 = S13.pt[1], Exp2 = S13.pt[2], Exp3 = S13.pt[3], Exp4 = NA,
    Exp5 = NA, RR = NA
  )

  S13pEffectSizes <- subset(ExperimentEffectSizes, ExperimentEffectSizes$Study == "S13" & Type == "p")
  ...
  # ************************************************************* Study 14: S14. Pfahl et al. 20054. Pfahl aggregated p-values but also used
  # values based on differences of differences. Revised analysis to be based on the reported differences to reduce problems with rounding
  # errors.

  S14data <- subset(ExpData, ExpData$Study == "S14")
  S14EffectSizes <- PrepareForMetaAnalysisGtoR(S14data$Mc, S14data$Mt, S14data$SDc, S14data$SDt, S14data$Nc, S14data$Nt)
  ...
  MAStats <- rbind(MAStats, data.frame(Study,
    Type = "P", Source = "Calc", mean = signif(S14.P, 4), pvalue = signif(S14.P.pvalue, 4), UB = NA, LB = NA,
    QE = NA, QEp = NA, RR = NA
  )) # issue: character instead of numeric values, e.g., in pvalue, solution: cbind -> data.frame /LM


  # There is no standard size for P values so we do not do a reproducibility test.  Put the reported P value resutls into MAStats
  S14MAPrep <- subset(MARepResShort, MARepResShort$Study == "S14" & MARepResShort$Type == "P")
  ...
  ESdata1 <- data.frame(
    Study = Study, Type = "dRM", Source = "Calc", Design = "PrePost", Exp1 = S14Hg.ES[1], Exp2 = S14Hg.ES[2], Exp3 = S14Hg.ES[3],
    Exp4 = NA, Exp5 = NA, RR = NA
  )

  # Obtain the reported dRM effect sizes
  S14HgRep <- subset(ExperimentEffectSizes, ExperimentEffectSizes$Study == "S14" & ExperimentEffectSizes$Type == "dRM")
  ...
  S14pRep <- subset(ExperimentEffectSizes, ExperimentEffectSizes$Study == "S14" & ExperimentEffectSizes$Type == "p")
  # Put the calculated p values into a temporary data.fromae
  ESdata1 <- data.frame(Study = Study, Type = "p", Source = "Calc", Design = "PrePost", Exp1 = signif(S14.tp[1], 4), Exp2 = signif(S14.tp[2]), Exp3 = signif(
    S14.tp[3],
    4
  ), Exp4 = NA, Exp5 = NA, RR = NA)
  ...
  # library('xtable') xtable(MAStats) xtable(ESdata)
  ...
  # xtable(MAStatsTP1) xtable(ESTP1res)

  # Report extra results for Study 8 xtable(GH2015extra)

  results <- list(MAStats = MAStats, ESdata = ESdata, MAStatsTP1 = MAStatsTP1, ESTP1res = ESTP1res, GH2015extra = GH2015extra)
  return(results)
}
